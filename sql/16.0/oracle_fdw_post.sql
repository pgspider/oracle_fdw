...
ALTER EXTENSION oracle_fdw ADD TEXT SEARCH CONFIGURATION public.custom_search;
...
--Testcase 936:
CREATE USER MAPPING FOR regress_view_owner SERVER oracle_srv OPTIONS (user 'test', password 'test');;
...
-- Oracle FDW does not support 'use_remote_estimate' option.
...
--Testcase 948:
EXPLAIN (COSTS OFF) SELECT * FROM v4;
-- Likewise, but with the query under an UNION ALL
--Testcase 949:
EXPLAIN (COSTS OFF) SELECT * FROM (SELECT * FROM v4 UNION ALL SELECT * FROM v4);
-- Should not get that error once a user mapping is created
--Testcase 950:
CREATE USER MAPPING FOR regress_view_owner_another SERVER oracle_srv OPTIONS (user 'test', password 'test');
--Testcase 951:
EXPLAIN (COSTS OFF) SELECT * FROM v4;
--Testcase 952:
EXPLAIN (COSTS OFF) SELECT * FROM (SELECT * FROM v4 UNION ALL SELECT * FROM v4);
--Testcase 953:
DROP USER MAPPING FOR regress_view_owner_another SERVER oracle_srv;
--Testcase 954:
...
-- oracle_fdw does not support batch insert
